...
1.  We need to add Prometheus middleware handler:
  ```
...
Prometheus provides a collector that can scrape and expose the mBeans of a JMX target.

It runs as a JavaAgent, exposing an HTTP server and serving metrics of the local JVM.

If the user wants to have an enabled hotspot monitor for JVM, change the enableHotspot value to true:
...
The Light framework provided docker-compose file (docker-compose-prometheus.yml) includes the Grafana image: image: grafana/grafana:4.6.3

So the user can open a browser to view  the Prometheus from Grafana
...
Please follow the following steps to set the Prometheus up in Grafana:
...
After Prometheus works as a backend dataSource for Grafana, the user can customize their own template or dashboard in Grafana for Prometheus metrics.


You can get more details here: http://docs.grafana.org/features/datasources/prometheus/